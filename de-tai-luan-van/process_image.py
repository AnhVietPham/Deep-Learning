...
def concat_image(images, rows, cols):
...
def cutout(image_origin, mask_origin, mask_size, size, mask_value='mean'):
    ...
    image[top:bottom, left:right, :].fill(0)
    image = resize(image, size)
    mask = resize(mask, size)
...
if __name__ == "__main__":
    ...
    full_original_path = os.path.join(original_path, dataset_name)
    ...
    images = glob(os.path.join(full_original_path, "images/", "*"))
    masks = glob(os.path.join(full_original_path, "masks/", "*"))
